...
object Util {
  ...
  def setLevel(options: DataSourceOptions, log: Logger): Unit = {
    val level = options.get(Opt.LOGLEVEL).orElse(Opt.LOGLEVELDEF) match {
...
object KdbCall {
  ...
  def call(hostind: Int, options: java.util.Map[String, String], arg1: Object, arg2: Object): Object = {
    ...
    if (platformTarget.length > 0) {
      ...
      val confirmReceive = con.k(queryObj) // This is the query id of the query being executed
...
object Opt {
  ...
  val QEXPR = "qexpr" // U: q expression that returns a table
  ...
  val BATCHINGSIZE = "batchingsize"; val BATCHINGSIZEDEF = 10000 // U: Number of rows to batch in one kdb+ write call
  ...
  val LOGLEVEL = "loglevel"; val LOGLEVELDEF = "warn"
}